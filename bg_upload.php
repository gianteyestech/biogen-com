...
// ─────────────────────────────────────────────────────────────────────────────
// CONFIGURATION: Secret key to authenticate requests from Next.js backend
// MUST match HOSTINGER_UPLOAD_SECRET in your Next.js .env.local file
// ─────────────────────────────────────────────────────────────────────────────
$SECRET_KEY = "biogen_media_secret_2026_change_me";
...
if (isset($_GET['action']) && $_GET['action'] === 'sync') {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if (!empty($pushedCode) && strpos($pushedCode, '<?php') !== false) {
            ...
            header("Content-Type: application/json");
            echo json_encode(["success" => true, "message" => "Hostinger bg_upload.php updated via direct push sync!"]);
            exit();
        }
    }

    // 2. GET Pull Sync from GitHub Raw
    $rawUrl = "https://raw.githubusercontent.com/gianteyestech/biogen-com/main/bg_upload.php";
    $latestCode = false;
    $curlErr = "";
    ...
    if ($latestCode && strpos($latestCode, '<?php') !== false) {
        ...
        header("Content-Type: application/json");
        echo json_encode(["success" => true, "message" => "Hostinger bg_upload.php self-updated to latest GitHub version!"]);
        exit();
    } else {
...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    $baseUrl = "https://assets.idealdryfruit.com";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (move_uploaded_file($file['tmp_name'], $targetPath)) {
        $publicUrl = "https://assets.idealdryfruit.com/" . $relSubPath . $customFilename;
        header("Content-Type: application/json");
        echo json_encode([
            "success" => true,
            "url" => $publicUrl,
